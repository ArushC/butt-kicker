...
exports.seed = async function(knex) {
  ...
  const existingMessages = await knex('positive_reinforcement_messages').select('streak_number', 'message');

  // Convert existing messages to a Set for quick lookup
  const existingMessagesSet = new Set(existingMessages.map(msg => JSON.stringify(msg)));

  // Filter out messages that already exist
  const newMessages = messagesData
      .filter(message => !existingMessagesSet.has(JSON.stringify(message)));

  // Insert new messages data into the database in batches
  if (newMessages.length > 0) {
      await batchInsert(knex, 'positive_reinforcement_messages', newMessages);
  }
